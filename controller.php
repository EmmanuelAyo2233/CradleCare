...
//Add immunization ...............................................................................................
if(isset($_POST['add-immunization'])) {
    // Assuming you have a function validate_data to sanitize your data.
    $firstname = validate_data($_POST['fname']);
    ...
    $patient_id = validate_data($_POST['patient_id']);
    $vaccine = validate_data($_POST['vaccine']);
    $administerdBy = validate_data($_POST['administerdBy']);
    $state = validate_data($_POST['state']);
    $lga = validate_data($_POST['lga']);
    $address = validate_data($_POST['address']);
    $status = validate_data($_POST['status']);
    $bloodgroup = validate_data($_POST['bloodgroup']);
    $note = validate_data($_POST['note']);
    $allergies = validate_data($_POST['allergies']);
    $nextDueDate = validate_data($_POST['nextDueDate']);

    $connect = new mysqli("localhost", "root", "", "hos");

    if ($connect->connect_error) {
        die("Connection failed: " . $connect->connect_error);
    }

    $join_date = date('Y-m-d'); // Use a format compatible with MySQL.

    // Use prepared statements to prevent SQL injection.
    $stmt = $connect->prepare("INSERT INTO immunization (patient_id, name, email, gender, dob, phoneNumber, genotype, blood, address, note, date_created, status, typeOfVaccine, adminsterdBy, state, lga, allegies, nextDueDate)
                               VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)");
    ...
    $stmt->bind_param("ssssssssssssssssss", $patient_id, $name, $email, $gender, $dob, $phoneNumber, $gynotype, $bloodgroup, $address, $note, $join_date, $status, $vaccine, $administerdBy, $state, $lga, $allergies, $nextDueDate);

    if ($stmt->execute()) {
        echo '<script>alert("Added Successfully"); window.location.href="immunisation.php";</script>';
    } else {
        echo 'Error: ' . $stmt->error. " ". $patient_id, $name, $email, $gender, $dob, $phoneNumber, $gynotype, $bloodgroup, $address, $note, $join_date, $status, $vaccine, $administerdBy, $state, $lga, $allergies;
    }
